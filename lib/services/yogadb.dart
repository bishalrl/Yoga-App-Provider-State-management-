...
import 'package:provider_statemanagement/model/model.dart';
...
''');
  ...
  Future<YogaSummary?>InsertYogaSum(YogaSummary yogasum)async{
    final db=await instance.database;
    final id=await db!.insert(YogaModel.YogaSummary,
     yogasum.toJson());
   ...
   Future<List<YogaSummary>>readAllYogaSum()async{
    ...
    final query_res= await db!.query(YogaModel.YogaSummary,
    orderBy:orderBy );
  ...
  Future<List<Yoga>>readAllYoga(String TableName)async{
    final db= await instance.database;
    final orderBy= '${YogaModel.IDName} ASC';
    final query_res= await db!.query(TableName,
    orderBy:orderBy );
